...
pub fn sum_multiples_of_3_and_5_simd(bound: u32) -> u32 {
    ...
    //This cast means bounds are limited to architecture pointer size. Is an 8bit CPU always limited
    ...
    (&data[..])
        ...
        .simd_reduce(u32s::splat(0), u32s::splat(0), |accumulator, value| {
            ...
            // we need to iterate over all the scalars in the packed vector, because our vectors do not have a uniform value.
            ...
            for idx in 0..value.width() {
                ...
                //[Docs](https://docs.rs/faster/0.3.0/faster/vecs/trait.Packed.html#tymethod.replace) state a signature of `usize` for `idx` but compiler complains that it should be u32. why?
                // is it because the method gets in-lined and `idx` is cast internally to u32?
...
mod tests {
    ...
    use problem_1::{sum_multiples_of_3_and_5, sum_multiples_of_3_and_5_parallel,
                    sum_multiples_of_3_and_5_simd};
